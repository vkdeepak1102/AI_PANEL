...
if len(sys.argv) >= 3:
    ...
    l1_path = "data/L1 DEMO.csv"
    l2_path = "data/L2 DEMO.csv"
...
# -------------------------------------------------
# CREATE valid_ids   MUST COME BEFORE USING IT
# -------------------------------------------------
valid_ids = set(l1_df["candidate_id"])

# -------------------------------------------------
# FILTER L2 USING valid_ids
# -------------------------------------------------
l2_df = l2_df[l2_df["candidate_id"].isin(valid_ids)]

# -------------------------------------------------
# NOW BUILD PANEL LOOKUP (AFTER FILTERING)
# -------------------------------------------------
panel_lookup = {}
...
print(f"L1 valid candidates: {len(l1_df)}")
...
for f in os.listdir("output/transcripts"):
    d = json.load(open(f"output/transcripts/{f}"))
    json.dump(
